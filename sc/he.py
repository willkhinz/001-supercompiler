...
def embed(a, b, depth=0, memo=None):
    ...
    if r is not None:
        return r
    res = _embed(a, b, depth, memo)
    memo[key] = res
    return res


def _embed(a, b, depth, memo):
    if depth > 400:
    ...
    if ta == "#cons":
        if tb != "#cons":
            return False
        coupled = (embed(a[1], b[1], depth + 1, memo)
                   and embed(a[2], b[2], depth + 1, memo))
        if coupled:
            return True
        # bounded dig into the tail: keeps embedding cost linear-ish while
        # still catching reordering inside short spines
        t = b[2]
        digs = 0
        while t[0] == "#cons" and digs < 24:
            if embed(a, t, depth + 1, memo):
                return True
            t = t[2]
            digs += 1
        return embed(a, t, depth + 1, memo)
    if ta == "#clo":
        if tb != "#clo" or a[1] != b[1] or len(a[2]) != len(b[2]):
            return False
        return all(embed(x, y, depth + 1, memo)
